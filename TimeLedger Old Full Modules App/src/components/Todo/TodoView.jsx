...
import { IconEdit, IconTrash, IconInfo, IconPlus, IconClose, IconSearch, IconList } from '../Common/Icons';
...
export default function TodoView() {
    ...
    */
    useEffect(() => {
        const handler = (e) => {
            if (e.key === 'n' || e.key === 'N') setIsTodoModalOpen(true);
    ...
    const formatDueDate = (dateString) => {
        ...
        // If YYYY-MM-DD string, construct local date to avoid timezone shifts
    ...
    const toggleSubtaskView = (id) => {
        setExpandedSubtasks(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
    };
    ...
    const renderTodoItem = (todo, compact = false) => {
        ...
        const isSubtasksExpanded = expandedSubtasks.includes(todo.id);
        ...
        return (
            ...
            >
                {/* Checkbox */}
                <div
                    className={`${styles.checkbox} ${hasIncompleteSubtasks ? styles.disabled : ''}`}
                    onClick={hasIncompleteSubtasks ? undefined : () => toggleTodo(todo.id)}
                ...
                <div className={styles.todoBody}>
                    <div className={styles.todoInlineRow}>
                        <div className={styles.taskPrimaryInfo}>
                            <span
                                className={`${styles.todoText} ${hasIncompleteSubtasks ? styles.textDisabled : ''}`}
                                onClick={hasIncompleteSubtasks ? undefined : () => toggleTodo(todo.id)}
                                style={{ cursor: hasIncompleteSubtasks ? 'not-allowed' : 'pointer' }}
                            >
                                {todo.text}
                            </span>
                            ...
                            {todo.subtasks?.length > 0 && (
                                <button
                                    className={`${styles.btnExpand} ${isSubtasksExpanded ? styles.expanded : ''}`}
                                    onClick={() => toggleSubtaskView(todo.id)}
                    ...
                    {/* NEW: subtasks */}
                    {todo.subtasks?.length > 0 && isSubtasksExpanded && (
    ...
    // Add task button (used in empty state)
    const addTaskButton = () => {
        return (
    ...
    return (
        <div className={styles.todoView}>
            ...
            {
                viewMode !== 'focus' && isTodoModalOpen && (
                    <div className={styles.modalOverlay} onClick={handleCancel}>
                        <form className={styles.inputArea} onSubmit={handleSubmit} onClick={(e) => e.stopPropagation()}>
                            ...
                            <button type="button" className={styles.btnCloseModal} onClick={handleCancel} title="Dismiss"><IconClose size={24} /></button>
