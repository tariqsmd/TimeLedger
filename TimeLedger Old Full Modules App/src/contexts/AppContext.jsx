...
import { sampleTodos, sampleTrackerEntries, sampleMonthlyGoals, sampleProjects } from '../data/sampleData';
...
export function AppProvider({ children }) {
    ...
    const loadSampleData = () => {
        if (window.confirm('This will replace your current data with sample data. Are you sure?')) {
            try {
                localStorage.setItem('timeBlockTodos', JSON.stringify(sampleTodos));
                localStorage.setItem('timeBlockTrackerData', JSON.stringify({ entries: sampleTrackerEntries }));
                localStorage.setItem('timeBlockMonthlyGoals', JSON.stringify(sampleMonthlyGoals));
                localStorage.setItem('mt_projects', JSON.stringify(sampleProjects));
                localStorage.setItem('timeBlockSchedules', JSON.stringify(scheduleData));

                // Force state update as well in case reload is delayed or prevented (though reload usually clears state)
                setTodos(sampleTodos);
                setTrackerData({ entries: sampleTrackerEntries });
                setMonthlyGoals(sampleMonthlyGoals);
                setProjects(sampleProjects);
                setCustomSchedules(scheduleData);
